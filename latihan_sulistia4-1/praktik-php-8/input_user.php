    ...
    if (empty($username) || empty($password)) {
        ...
        if ($result !=0){
            ...
            $query = "insert into pengguna (username, password)" .  "VALUES ('$username','$password')";
                ...
                mysqli_query($conn, $query) or die('Eror, query failed' . mysqli_error($conn));
                mysqli_close($conn);

                echo "Add User Administrator '$username' SUCCSES";
                exit;
...
<html>
    <head>
        <title>Menambah User Admin</title>
    </head>
    <body>
       ...
       <table width="90%" border="0" align="center" cellpadding="2" cellsapcing="2" class="content">
        <tr bgcolor="#FFDFFA">
        <td colspan="3">
